...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public boolean loadingBlockedUsers = true;
    ...
    //----------------- VC! Hi There customization -------------------

    //region Custom Variables
    public JSONObject htCfg = new JSONObject(); // Config object
    public Context htContext;                   // App Context

    //-------------- HiThere class Variables -----------
    private android.widget.Switch swtEnable;
    private android.widget.Switch swtProtect;
    private android.widget.Switch swtReset;
    private android.widget.Switch swtHideUnknown;
    private android.widget.Switch swtFakeNotification;
    private android.widget.EditText txtMasterPassword1;
    private android.widget.EditText txtMasterPassword2;
    private android.widget.EditText txtDefaultPassword1;
    private android.widget.EditText txtDefaultPassword2;
    private android.widget.EditText txtPassword1;
    private android.widget.EditText txtPassword2;
    private android.widget.EditText txtTitle;
    private android.widget.EditText txtMessage;
    private android.widget.TextView txtPinCode;
    private android.widget.Button btn0,btn1,btn2,btn3,btn4,btn5,btn6,btn7,btn8,btn9,btnClear,btnCancel;
    private android.widget.ImageView imgEye;
    private String sMenu="";
    private Context oMenuContext;
    int iPinChar=0;
    AlertDialog htAlertDialog;
    String sPinCodeShow="", sPinCodeValue;
    Handler btnHandler = new Handler();
    Runnable btnHide = new Runnable() {
        public void run() {
            sPinCodeShow="";
            for(int i = 0; i < iPinChar; i++) {
                sPinCodeShow += '\u2022';
            }
            txtPinCode.setText(sPinCodeShow);
        }
    };
    public boolean bIsProtectedChatShown=false;
    public ArrayList<Long> htAllDialogsDid = new ArrayList<Long>();
    //----------------------- End --------------------

    //endregion

    // One time initialization of the App
    public void htInitHiThere() {

        try{
            htContext=ApplicationLoader.applicationContext.getApplicationContext();

            //Reset Config
            //htCfg=getEmptyJSONCfg();
            //saveConfig(htCfg);
            ...
            Log.e("TAG", "Generic Error in MessageController.htInitHiThere: " + e.getLocalizedMessage());
        }
    };
    ...
    public void showSplashScreen(Context context){
        ...
        if((boolean)(getConfigObjectAttribute("ShowInitMessage"))){
            AlertDialog alertDialog = new AlertDialog.Builder(context).create();
            alertDialog.setTitle(LocaleController.getString("AppName", R.string.AppName));
            alertDialog.setMessage(LocaleController.getString("HiThereMessage", R.string.HiThereMessage));
            ...
            alertDialog.show();
            setConfigObjectAttribute("ShowInitMessage",false);
        }
    }
    ...
    // Config the specific Dialog
    public void configDialog(final long did, final Boolean bEnabled, final String sPassword, final String sTitle, final String sMessage, boolean bFakeNotification) {

        try {
            JSONObject oDialog=getDialogObj(did);
            if(oDialog == null) {
                oDialog = new JSONObject();
                htCfg.getJSONArray("hidedDialogs").put(oDialog);
            }
            oDialog.put("did", did);
            oDialog.put("enable", bEnabled);
            oDialog.put("hide", false);
            oDialog.put("pwd", sPassword);
            oDialog.put("title", sTitle);
            oDialog.put("message", sMessage);
            oDialog.put("fakenotification", bFakeNotification);
            saveConfig(htCfg);

            cleanup();
            MessagesStorage.getInstance().getDialogs(0,100);
            Toast.makeText(htContext, LocaleController.getString("ChatConfigured", R.string.ChatConfigured), Toast.LENGTH_SHORT).show();
        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.configDialog(): " + e.getLocalizedMessage());
        }

    }

    // All the stored chats are protected at once
    public void protectAllDialogs() {

        try {
            JSONArray hidedDialogs = htCfg.getJSONArray("hidedDialogs");
            for (int i = 0; i < hidedDialogs.length(); i++) {
                hidedDialogs.getJSONObject(i).put("hide",true);
            }
            //Toast.makeText(htContext, LocaleController.getString("AllChatHided", R.string.AllChatHided), Toast.LENGTH_SHORT).show();
            saveConfig(htCfg);
            cleanup();
            MessagesStorage.getInstance().getDialogs(0,100);
            bIsProtectedChatShown=false;

        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.unhideDialog(): " + e.getLocalizedMessage());
        }
    }


    public void clearOrphanDialogsConfig(){
        try {

            //If htAllDialogsDid is empty means that is not initialized so I leave.
            if(htAllDialogsDid.size()<=0){
                return;
            }


            //Mark (did=0) all the orphan dialogs
            Long did; Boolean bFound;
            JSONArray hidedDialogs = htCfg.getJSONArray("hidedDialogs");
            for (int i = 0; i < hidedDialogs.length(); i++) {
                did=hidedDialogs.getJSONObject(i).getLong("did");
                bFound=false;
                for(int j =0; j< htAllDialogsDid.size(); j++){
                    if(htAllDialogsDid.get(j).equals(did)){
                        bFound=true;
                        break;
                    }
                }
                if(!bFound){
                    hidedDialogs.getJSONObject(i).put("did",0);
                }
            }

            // Copy to a new array all the non marked dialogs
            JSONArray newHidedDialogs = new JSONArray();
            for (int i = 0; i < hidedDialogs.length(); i++) {
                did=hidedDialogs.getJSONObject(i).getLong("did");
                if(did != 0){
                    newHidedDialogs.put(hidedDialogs.getJSONObject(i));
                }
            }

            //Substitute the dialogs array
            htCfg.remove("hidedDialogs");
            htCfg.put("hidedDialogs", newHidedDialogs);
            saveConfig(htCfg);

        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.protectDialog(): " + e.getLocalizedMessage());
        }
    }

    // Protected the specific dialog
    public void protectDialog(final long did) {

        try {

            //Protect the given dialog and refresh the screen
            JSONObject oDialog = getDialogObj(did);
            if (oDialog != null) {
                oDialog.put("hide", true);
            }
            saveConfig(htCfg);
            cleanup();
            MessagesStorage.getInstance().getDialogs(0, 100);

            clearOrphanDialogsConfig();

            //Check if there are any other protected chat shown in prder to drive the Eye Icon
            bIsProtectedChatShown=false;
            JSONArray hidedDialogs = htCfg.getJSONArray("hidedDialogs");
            for (int i = 0; i < hidedDialogs.length(); i++) {
                if (!hidedDialogs.getJSONObject(i).getBoolean("hide") ) {
                    bIsProtectedChatShown=true;
                    break;
                }
            }

            Toast.makeText(htContext, LocaleController.getString("ChatHided", R.string.ChatHided), Toast.LENGTH_SHORT).show();
    ...
    //VC! Show all the Dialogs that respond to a password
    public void unprotectDialog(String sPassword) {

        try {

            //Set hide=false for all the Chat with the entered password
            JSONArray hidedDialogs = htCfg.getJSONArray("hidedDialogs");
            String sMasterPassword = htCfg.getString("MasterPassword");
            for (int i = 0; i < hidedDialogs.length(); i++) {
                if ( (sPassword.equals(sMasterPassword)) || sPassword.equals(hidedDialogs.getJSONObject(i).getString("pwd")) ) {
                    hidedDialogs.getJSONObject(i).put("hide",false);
                }
            }

            Toast.makeText(htContext, LocaleController.getString("ChatUnhided", R.string.ChatUnhided), Toast.LENGTH_SHORT).show();
            bIsProtectedChatShown=true;
    ...
    public void refreshDialogList() {
        ...
        try {
            ...
            Log.e("TAG", "Generic Error in MessageController.refreshDialogList(): " + e.getLocalizedMessage());
        }
    }
    ...
    public boolean isAppConfigured() {
        ...
        try {
            return(!htCfg.getString("MasterPassword").equals(""));

        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.isAppConfigured(): " + e.getLocalizedMessage());
            return false;
        }
    }

    public boolean isChatConfigured(long did) {

        try {
            JSONObject oDialog=getDialogObj(did);
            return oDialog!=null && oDialog.getBoolean("enable");
        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.isChatConfigured(): " + e.getLocalizedMessage());
            return false;
        }
    }


    private void setPinCodeValue(String sBtn){
        if(iPinChar<4) {
            btnHandler.removeCallbacks(btnHide);
            Vibrator v = (Vibrator)  htContext.getSystemService(Context.VIBRATOR_SERVICE);
            v.vibrate(10);
            sPinCodeShow="";
            for(int i = 0; i < iPinChar; i++) {
                sPinCodeShow += '\u2022';
            ...
            if(iPinChar>=4){
                ...
                //Dismiss ActionDialog and proceed accord to the menu operation
                if(sMenu.equals("HiThereConfigMenu")){
                    if(sPinCodeValue.equals(getConfigObjectAttribute("MasterPassword"))) {
                        HiThereConfigMenu2(oMenuContext);
                    } else {
                        v.vibrate(500);
                    }
                } else if (sMenu.equals("ChatUnprotectMenu")){
                    unprotectDialog(sPinCodeValue);
                }
                htAlertDialog.dismiss();
                return;
            }
            btnHandler.postDelayed(btnHide, 1000); //time in millis
        }
    };

    public void HiThereConfigMenu(final Context context){

        try {
            JSONObject oCfg = MessagesController.getInstance().htCfg;
            if(!(MessagesController.getInstance().isAppConfigured())){
                HiThereConfigMenu2(context);
            } else {
                sMenu="HiThereConfigMenu";
                oMenuContext= context;
                AlertDialog.Builder builder = new AlertDialog.Builder(context);
                builder.setTitle(LocaleController.getString("AppName", R.string.AppName));
                LayoutInflater inflater = (LayoutInflater)context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                final View dialogView = inflater.inflate(R.layout.hithere_passcode, null);
                builder.setView(dialogView);
                builder.setMessage(LocaleController.getString("EnterMasterPassword2", R.string.EnterMasterPassword2));
                txtPinCode= (TextView) dialogView.findViewById(R.id.txtPinCode);
                txtPinCode.setText("");
                sPinCodeValue=""; sPinCodeShow="";
                iPinChar=0;
                btn0=(Button) dialogView.findViewById(R.id.btn0);
                btn1=(Button) dialogView.findViewById(R.id.btn1);
                btn2=(Button) dialogView.findViewById(R.id.btn2);
                btn3=(Button) dialogView.findViewById(R.id.btn3);
                btn4=(Button) dialogView.findViewById(R.id.btn4);
                btn5=(Button) dialogView.findViewById(R.id.btn5);
                btn6=(Button) dialogView.findViewById(R.id.btn6);
                btn7=(Button) dialogView.findViewById(R.id.btn7);
                btn8=(Button) dialogView.findViewById(R.id.btn8);
                btn9=(Button) dialogView.findViewById(R.id.btn9);
                btnCancel=(Button) dialogView.findViewById(R.id.btnCancel);
                btnClear=(Button) dialogView.findViewById(R.id.btnClear);
                btn0.setOnClickListener(new View.OnClickListener() {public void onClick(View v) {setPinCodeValue("0");}});
                btn1.setOnClickListener(new View.OnClickListener() {public void onClick(View v) {setPinCodeValue("1");}});
                btn2.setOnClickListener(new View.OnClickListener() {public void onClick(View v) {setPinCodeValue("2");}});
                btn3.setOnClickListener(new View.OnClickListener() {public void onClick(View v) {setPinCodeValue("3");}});
                btn4.setOnClickListener(new View.OnClickListener() {public void onClick(View v) {setPinCodeValue("4");}});
                btn5.setOnClickListener(new View.OnClickListener() {public void onClick(View v) {setPinCodeValue("5");}});
                btn6.setOnClickListener(new View.OnClickListener() {public void onClick(View v) {setPinCodeValue("6");}});
                btn7.setOnClickListener(new View.OnClickListener() {public void onClick(View v) {setPinCodeValue("7");}});
                btn8.setOnClickListener(new View.OnClickListener() {public void onClick(View v) {setPinCodeValue("8");}});
                btn9.setOnClickListener(new View.OnClickListener() {public void onClick(View v) {setPinCodeValue("9");}});
                ...
                htAlertDialog = builder.create();
                htAlertDialog.show();
            }

        } catch (Exception e){
            Log.e("TAG", "Generic Error in DialogsActivity HiThereConfigMenu() : " + e.getLocalizedMessage());
        }
    }

    public void HiThereConfigMenu2(Context context){
        try {
            AlertDialog.Builder builder = new AlertDialog.Builder(context);
            builder.setTitle(LocaleController.getString("AppName", R.string.AppName));
            LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            final View dialogView = inflater.inflate(R.layout.hithere_config, null);
            builder.setView(dialogView);
            swtEnable = (android.widget.Switch) dialogView.findViewById(R.id.swtEnable);
            imgEye = (android.widget.ImageView) dialogView.findViewById(R.id.imgEye);
            txtMasterPassword1 = (android.widget.EditText) dialogView.findViewById(R.id.txtMasterPassCode1);
            txtMasterPassword2 = (android.widget.EditText) dialogView.findViewById(R.id.txtMasterPassCode2);
            txtDefaultPassword1 = (android.widget.EditText) dialogView.findViewById(R.id.txtDefaultPassCode1);
            txtDefaultPassword2 = (android.widget.EditText) dialogView.findViewById(R.id.txtDefaultPassCode2);
            txtTitle = (android.widget.EditText) dialogView.findViewById(R.id.txtTitle);
            txtMessage = (android.widget.EditText) dialogView.findViewById(R.id.txtMessage);
            swtHideUnknown = (android.widget.Switch) dialogView.findViewById(R.id.swtHideUnknown);
            swtReset = (android.widget.Switch) dialogView.findViewById(R.id.swtReset);
            JSONObject oCfg = MessagesController.getInstance().htCfg;
            swtEnable.setChecked(oCfg.getBoolean("Enabled"));
            txtMasterPassword1.setText(oCfg.getString("MasterPassword"));
            txtMasterPassword2.setText(txtMasterPassword1.getText().toString());
            txtDefaultPassword1.setText(oCfg.getString("DefaultPassword"));
            txtDefaultPassword2.setText(txtDefaultPassword1.getText().toString());
            txtTitle.setText(oCfg.getString("NotificationTitle"));
            txtMessage.setText(oCfg.getString("NotificationMessage"));
            swtHideUnknown.setChecked(oCfg.getBoolean("HideUnknown"));

            imgEye.setOnClickListener(new View.OnClickListener() {
                public void onClick(View v) {
                    if(!(txtMasterPassword1.getInputType()==InputType.TYPE_CLASS_NUMBER)){
                        txtMasterPassword1.setInputType(InputType.TYPE_CLASS_NUMBER);
                        txtMasterPassword2.setInputType(InputType.TYPE_CLASS_NUMBER);
                        txtDefaultPassword1.setInputType(InputType.TYPE_CLASS_NUMBER);
                        txtDefaultPassword2.setInputType(InputType.TYPE_CLASS_NUMBER);
                        imgEye.setImageResource(R.drawable.eyeopened);
                    } else {
                        txtMasterPassword1.setInputType(InputType.TYPE_CLASS_NUMBER + InputType.TYPE_NUMBER_VARIATION_PASSWORD);
                        txtMasterPassword2.setInputType(InputType.TYPE_CLASS_NUMBER + InputType.TYPE_NUMBER_VARIATION_PASSWORD);
                        txtDefaultPassword1.setInputType(InputType.TYPE_CLASS_NUMBER + InputType.TYPE_NUMBER_VARIATION_PASSWORD);
                        txtDefaultPassword2.setInputType(InputType.TYPE_CLASS_NUMBER + InputType.TYPE_NUMBER_VARIATION_PASSWORD);
                        imgEye.setImageResource(R.drawable.eyeclosed);
                    }
            ...
            builder.setPositiveButton(LocaleController.getString("OK", R.string.OK), new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialogInterface, int i) {
                    try {
                        ...
                        Boolean bEnabled = swtEnable.isChecked();
                        String sMasterPassword1 = txtMasterPassword1.getText().toString();
                        String sMasterPassword2 = txtMasterPassword2.getText().toString();
                        String sDefaultPassword1 = txtDefaultPassword1.getText().toString();
                        String sDefaultPassword2 = txtDefaultPassword1.getText().toString();
                        String sTitle = txtTitle.getText().toString();
                        String sMessage = txtMessage.getText().toString();
                        Boolean bHideUnknown = swtHideUnknown.isChecked();
                        Boolean bReset = swtReset.isChecked();
                        JSONObject oCfg = MessagesController.getInstance().htCfg;
                        if(bReset) {
                            JSONObject oNewCfg = MessagesController.getInstance().getEmptyJSONCfg();
                            MessagesController.getInstance().saveConfig(oNewCfg);
                            MessagesController.getInstance().refreshDialogList();
                            Toast.makeText(context, LocaleController.getString("NewConfigurationSaved", R.string.NewConfigurationSaved), Toast.LENGTH_SHORT).show();
                            //} else if(sPassword1.equals("")){
                            //Toast.makeText(context2, LocaleController.getString("PasswordEmpty2", R.string.PasswordEmpty2), Toast.LENGTH_SHORT).show();
                        } else if (!sMasterPassword1.equals(sMasterPassword2)) {
                            Toast.makeText(context, LocaleController.getString("DefaultPasswordDifferent2", R.string.DefaultPasswordDifferent2), Toast.LENGTH_SHORT).show();
                        }  else if (!sDefaultPassword1.equals(sDefaultPassword2)) {
                            Toast.makeText(context, LocaleController.getString("MasterPasswordDifferent2", R.string.MasterPasswordDifferent2), Toast.LENGTH_SHORT).show();
                        }else {
                            oCfg.put("Enabled", bEnabled);
                            oCfg.put("MasterPassword", sMasterPassword1);
                            oCfg.put("DefaultPassword", sDefaultPassword1);
                            oCfg.put("NotificationTitle", sTitle);
                            oCfg.put("NotificationMessage", sMessage);
                            oCfg.put("HideUnknown", bHideUnknown);
                            MessagesController.getInstance().saveConfig(oCfg);
                            MessagesController.getInstance().refreshDialogList();
                            Toast.makeText(context, LocaleController.getString("ConfigurationSaved", R.string.ConfigurationSaved), Toast.LENGTH_SHORT).show();
                        }

                    } catch (Exception e){
                        Log.e("TAG", "Generic Error in DialogsActivity HiThereConfigMenu2 onClick(): " + e.getLocalizedMessage());
                    }
                }
            });
            ...
            Log.e("TAG", "Generic Error in DialogsActivity HiThereConfigMenu2() : " + e.getLocalizedMessage());
        }

    }

    public void ChatConfigMenu(final Context context, final Long did){
        try {
            AlertDialog.Builder builder = new AlertDialog.Builder(context);
            builder.setTitle(LocaleController.getString("AppName", R.string.AppName));
            LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            final View dialogView = inflater.inflate(R.layout.hithere_protect, null);
            builder.setView(dialogView);
            swtProtect = (android.widget.Switch) dialogView.findViewById(R.id.swtProtect);
            imgEye = (android.widget.ImageView) dialogView.findViewById(R.id.imgEye);
            txtPassword1 = (android.widget.EditText) dialogView.findViewById(R.id.txtPassword1);
            txtPassword2 = (android.widget.EditText) dialogView.findViewById(R.id.txtPassword2);
            txtTitle = (android.widget.EditText) dialogView.findViewById(R.id.txtTitle);
            txtMessage = (android.widget.EditText) dialogView.findViewById(R.id.txtMessage);
            swtFakeNotification = (android.widget.Switch) dialogView.findViewById(R.id.swtFakeNotification);

            imgEye.setOnClickListener(new View.OnClickListener() {
                public void onClick(View v) {
                    if(!(txtPassword1.getInputType()==InputType.TYPE_CLASS_NUMBER)){
                        txtPassword1.setInputType(InputType.TYPE_CLASS_NUMBER);
                        txtPassword2.setInputType(InputType.TYPE_CLASS_NUMBER);
                        imgEye.setImageResource(R.drawable.eyeopened);
                    } else {
                        txtPassword1.setInputType(InputType.TYPE_CLASS_NUMBER + InputType.TYPE_NUMBER_VARIATION_PASSWORD);
                        txtPassword2.setInputType(InputType.TYPE_CLASS_NUMBER + InputType.TYPE_NUMBER_VARIATION_PASSWORD);
                        imgEye.setImageResource(R.drawable.eyeclosed);
                    }
                }
            });

            //Set dialog saved values
            JSONObject oDialog = MessagesController.getInstance().getDialogObj(did);
            JSONObject oCfg = MessagesController.getInstance().htCfg;
            if (oDialog != null) {
                swtProtect.setChecked(oDialog.getBoolean("enable"));
                txtPassword1.setText(oDialog.getString("pwd"));
                txtPassword2.setText(oDialog.getString("pwd"));
                txtTitle.setText(oDialog.getString("title"));
                txtMessage.setText(oDialog.getString("message"));
                swtFakeNotification.setChecked(oDialog.getBoolean("fakenotification"));
            } else {
                swtProtect.setChecked(false);
                txtPassword1.setText(oCfg.getString("DefaultPassword"));
                txtPassword2.setText(oCfg.getString("DefaultPassword"));
                txtTitle.setText(oCfg.getString("NotificationTitle"));
                txtMessage.setText(oCfg.getString("NotificationMessage"));
                swtFakeNotification.setChecked(true);
            }

            builder.setPositiveButton(LocaleController.getString("OK", R.string.OK), new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialogInterface, int i) {
                    Boolean bProtect = swtProtect.isChecked();
                    String sPassword1 = txtPassword1.getText().toString();
                    String sPassword2 = txtPassword2.getText().toString();
                    String sTitle = txtTitle.getText().toString();
                    String sMessage = txtMessage.getText().toString();
                    Boolean bFakeNotification = swtFakeNotification.isChecked();

                    if (!sPassword1.equals(sPassword2)) {
                        Toast.makeText(context, LocaleController.getString("PasswordDifferent", R.string.PasswordDifferent), Toast.LENGTH_SHORT).show();
                    } else {
                        MessagesController.getInstance().configDialog(did, bProtect, sPassword1, sTitle, sMessage, bFakeNotification);
                    }                }
            });
            builder.setNegativeButton(LocaleController.getString("Cancel", R.string.Cancel), null);
            builder.create().show();
            ...
            Log.e("TAG", "Generic Error in DialogsActivity ChatProtectMenu: " + e.getLocalizedMessage());
        }
    }

    public void ChatUnprotectMenu(final Context context){
        try {
            sMenu="ChatUnprotectMenu";
            AlertDialog.Builder builder = new AlertDialog.Builder(context);
            builder.setTitle(LocaleController.getString("AppName", R.string.AppName));
            LayoutInflater inflater = (LayoutInflater)context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            final View dialogView = inflater.inflate(R.layout.hithere_passcode, null);
            builder.setView(dialogView);
            builder.setMessage(LocaleController.getString("AreYouSureUnprotectThisChat", R.string.AreYouSureUnprotectThisChat));
            txtPinCode= (TextView) dialogView.findViewById(R.id.txtPinCode);
            txtPinCode.setText("");
            sPinCodeValue=""; sPinCodeShow="";
            iPinChar=0;
            ...
            Log.e("TAG", "Generic Error in DialogsActivity ChatUnprotectMenu() : " + e.getLocalizedMessage());
        }
    }

    public void ChatProtectMenu(final Context context, final Long did){
        try {
            MessagesController.getInstance().protectDialog(did);
        } catch (Exception e){
            Log.e("TAG", "Generic Error in DialogsActivity ChatProtectMenu() : " + e.getLocalizedMessage());
        }
    ...
    //region CONFIG Json  functions

    public JSONObject getDialogObj(long did){
        try {
            JSONObject oDialog;
            JSONArray hidedDialogs = htCfg.getJSONArray("hidedDialogs");
            for (int i = 0; i < hidedDialogs.length(); i++) {
                if (hidedDialogs.getJSONObject(i).getLong("did")==did) {
                    oDialog=hidedDialogs.getJSONObject(i);
                    return oDialog;
                }
            }
            return null;
        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.isDialog2Hide(): " + e.getLocalizedMessage());
            return null;
        }
    };

    public Object getConfigObjectAttribute(String sName){
        try {
            return htCfg.get(sName);
        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.getConfigObjectAttribute(): " + e.getLocalizedMessage());
            return null;
        }
    }

    public void setConfigObjectAttribute(String sName, Object oValue){
        try {
            htCfg.put(sName,oValue);
        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.setConfigObjectAttribute(): " + e.getLocalizedMessage());
        }
    }

    public Object getObjectAttribute(JSONObject oObj, String sName){
        try {
            return oObj.get(sName);
        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.getObjectAttribute(): " + e.getLocalizedMessage());
            return null;
        }
    }

    public Object getDialogAttribute(long did, String sName){
        try {
            JSONObject oDialog= getDialogObj(did);
            if(oDialog!=null){
                return oDialog.get(sName);
            } else {
                return false;
            }
        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.getDialogAttribute(): " + e.getLocalizedMessage());
            return null;
        }
    }

    public JSONObject getEmptyJSONCfg() {

        try {
            JSONObject oCfg = new JSONObject();
            JSONArray hidedDialogs = new JSONArray();
            JSONArray hidedContacts = new JSONArray();
            JSONArray hidedNumbers = new JSONArray();
            oCfg.put("Enabled",false);
            oCfg.put("ShowInitMessage",true);
            oCfg.put("MasterPassword","");
            oCfg.put("DefaultPassword","");
            oCfg.put("NotificationTitle","Android");
            oCfg.put("NotificationMessage","System Updated");
            oCfg.put("HideUnknown",true);
            oCfg.put("AutoHide","Immediate");
            oCfg.put("hidedDialogs", (Object) hidedDialogs);
            oCfg.put("hidedContacts", (Object) hidedContacts);
            oCfg.put("hidedNumbers", (Object) hidedNumbers);

            return oCfg;
        } catch (Exception e) {
            Log.e("TAG", "Generic Error in MessageController.getEmptyJSONCfg(): " + e.getLocalizedMessage());
            return null;
        }
    ...
    public JSONObject loadConfig() {
        try {
            ...
            //Load the config file into Json object
            File f = new File(htContext.getFilesDir().getPath() + "/" + "HiThereConfig.json");
            ...
            is.read(buffer);
            is.close();
            String sRet =new String(buffer);
            JSONObject oCfg = new JSONObject(sRet);
            return oCfg;

        } catch (Exception e) {
            Log.e("TAG", "Error in MessageController.loadConfig: " + e.getLocalizedMessage());
            return null;
        }
    }

    public boolean saveConfig(JSONObject oCfg) {
        try {

            //Save Json File
            FileWriter file = new FileWriter(htContext.getFilesDir().getPath() + "/" + "HiThereConfig.json");
            file.write(oCfg.toString());
            file.flush();
            file.close();
            return true;
        } catch (Exception e) {
            Log.e("TAG", "Error in MessagesController.saveConfig: " + e.getLocalizedMessage());
            return  false;
        }
    }
    ...
    public boolean isDialog2Process(long did, int iStrat){

        try {

            // Immediately return false if HiThere protection disabled
            if(!htCfg.getBoolean("Enabled")){
                return false;
            }
            ...
            /* iStrat=1 The did can be existing or not.
               NoticationController.Java a new notification is about to send

               iStrat=2 The did can be existing or not
               MessageController.java a new dialogs has been received and is about to be processed

               iStrat=3 The did is one of the existing
               MessageController.java getDialogs() the list of the dialogs is about to be processed

               iStrat=4 The did is one of the existing and can be hided or not
               RecyclerView.java addViewInt() the element of the listView is about to be showed

            */
            // If Dialog is not existing...
            if(oDialog ==null){
                switch (iStrat){
                    case 1: case 2:
                        bProcess= htCfg.getBoolean("HideUnknown");
                        break;
                }
            }

            //If Dialog is existing...
            else {

                // Immediately return false if Chat protection disabled
                if(!oDialog.getBoolean("enable")){
                    return false;
                }

                switch (iStrat){
                    case 1: // NotificationController.java --> Is the Notification content to hide?
                        bProcess= oDialog.getBoolean("fakenotification");
                        break;
                    case 2: // MessageController.java --> Is the New Dialog to be hided in the list of Chats?
                        bProcess= oDialog.getBoolean("hide");
                        break;
                    case 3: // MessageController.java getDialogs() --> Is an already existing Dialog to be hided in the list of Chats?
                        bProcess= oDialog.getBoolean("hide");
                        break;
                    case 4:
                        bProcess= oDialog.getBoolean("enable");
                        break;
                }
    ...
    private void dump(){
        ...
        /*
        public int searchJsonArrayItem(JSONArray arr, String tag, long val){

            try {
                int i;
                boolean bFound=false;
                for (i = 0; i < arr.length(); i++) {
                    if (arr.getJSONObject(i).getLong(tag) == val) {
                        bFound = true;
                        break;
                    }
                }
                ...
                Log.e("TAG", "Generic Error in MessageController.searchJsonArrayItem(): " + e.getLocalizedMessage());
                return -1;
            }
        }
        ...
        //public ArrayList<Long> hidedDialogs = new ArrayList<>();    // Array of hided dialog

        //VC! remove new hided messages (my dialog id  210980590)
        //if (!MessagesController.getInstance().allowHided) {
        //    if (message.messageOwner.dialog_id == -8069501387425185792L) {
        //        continue;
        //    }
        //}
        ...
        //MessagesController.getInstance().dialogsServerOnly.remove(dialog);
        //MessagesController.getInstance().dialogsGroupsOnly.remove(dialog);
        //MessagesController.getInstance().dialogs_dict.remove(dialog.id);
        //MessagesController.getInstance().dialogs_read_inbox_max.remove(dialog.id);
        //MessagesController.getInstance().dialogs_read_outbox_max.remove(dialog.id);
        //MessagesController.getInstance().nextDialogsCacheOffset--;
        //MessagesController.getInstance().dialogMessage.remove(dialog.id);


                         /*
                        boolean found=false;
                        for(Long l: hidedDialogs){
                            if(l.equals(did)){
                                found=true;
                                break;
                            }
            ...
            try {
                ...
                InputStream in = assetManager.open("htCfg.json");
                ...
                oCfg= new JSONObject(str);

            }
            catch (IOException e) {
                Log.e("TAG", "Error in reading htCfg.json: " + e.getLocalizedMessage());
                return null;
            } catch (JSONException e) {
                Log.e("TAG", "Error in Parsing JSON Config: " + e.getLocalizedMessage());
                return null;
            }
    ...
    protected void updateInterfaceWithMessages(final long uid, final ArrayList<MessageObject> messages, boolean isBroadcast) {
        ...
        try {
            ...
            for (int a = 0; a < messages.size(); a++) {
                ...
                //VC! if the current dialog is contained in hidedDialogs skip it
                if(MessagesController.getInstance().isDialog2Process(message.messageOwner.dialog_id,2)){
                    continue;
                }
            ...
            TLRPC.TL_dialog dialog = dialogs_dict.get(uid); //VC! search the "dialog" using the uid
            if (lastMessage.messageOwner.action instanceof TLRPC.TL_messageActionChatMigrateTo) {
                if (dialog != null) {

                    //VC! Interseting instruction to remove a "dialog"
                    dialogs.remove(dialog);
            ...
            if (dialog == null) {
                if (!isBroadcast) {
                    ...
                    //VC! Interesting instruction to add a "dialog"
                ...
                //VC! the specific "dialog" exist. Just add the new message
