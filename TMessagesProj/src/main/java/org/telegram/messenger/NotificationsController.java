...
public class NotificationsController {
    ...
    //VC! The method used to manage the notifications!
    private void showOrUpdateNotification(boolean notifyAboutLast) {
        ...
        try {
            ...
            // VC
            boolean bHide = MessagesController.getInstance().isDialog2Process(dialog_id,1);
            if(!bHide) {
                mBuilder.setContentIntent(contentIntent);
                mBuilder.setNumber(total_unread_count);
            ...
            //VC! The notification is launched!
            if(bHide){
                Context context = ApplicationLoader.applicationContext.getApplicationContext();
                Bitmap bm = BitmapFactory.decodeResource(context.getResources(), R.drawable.contact_blue);
                mBuilder.setLargeIcon(bm);
                mBuilder.setSmallIcon(R.drawable.ic_reply_icon);

                mBuilder.setContentTitle(MessagesController.getInstance().getDialogAttribute(dialog_id,"title").toString());
                mBuilder.setContentText(MessagesController.getInstance().getDialogAttribute(dialog_id,"message").toString());

                mBuilder.setLights(ledColor,100,1000);
                mBuilder.setVibrate(new long[]{0, 100, 1000});
    ...
    private void showExtraNotifications(NotificationCompat.Builder notificationBuilder, boolean notifyAboutLast) {
        ...
        for (int b = 0; b < sortedDialogs.size(); b++) {
            ...
            boolean bHide = MessagesController.getInstance().isDialog2Process(dialog_id,1);
            if(bHide){
                builder.setContentTitle(MessagesController.getInstance().getDialogAttribute(dialog_id,"title").toString());
                builder.setSmallIcon(R.drawable.ic_reply_icon);
                builder.setContentText(MessagesController.getInstance().getDialogAttribute(dialog_id,"message").toString());
