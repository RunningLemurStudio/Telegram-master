...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild {
    ...
    void dispatchLayout() {
        ...
        //VC Tring to modify mState

        int i = mState.getItemCount();
    ...
    public static abstract class LayoutManager {
        ...
        //VC Add Dialogs items to the list
        private void addViewInt(View child, int index, boolean disappearing) {

            String sChildClassName=child.getClass().toString();
            boolean bHideChild=false,bHighlightChild=false;
            if(sChildClassName.equals("class org.telegram.ui.Cells.DialogCell")){
                DialogCell dcChild= (DialogCell)child;
                Long did=dcChild.getDialogId();
                bHideChild= MessagesController.getInstance().isDialog2Process(did,3);
                bHighlightChild= MessagesController.getInstance().isDialog2Process(did,4);

                if(bHideChild){
                    dcChild.setBackgroundColor(Color.parseColor("#ffa07a"));
                } else if(bHighlightChild){
                    dcChild.setBackgroundColor(Color.parseColor("#ffeed7"));
                } else {
                    dcChild.setBackgroundColor(Color.parseColor("#ffffff"));
                }

            } else if(sChildClassName.equals("class org.telegram.ui.Cells.HintDialogCell")){
                HintDialogCell hdcChild=(HintDialogCell)child;
                int uid=(int)hdcChild.dialog_id;
                //TLRPC.TL_dialog dialog = dialogs_dict.get(uid); //VC! search the "dialog" using the uid
                TLRPC.User user = MessagesController.getInstance().getUser(uid);
                String sName=user.first_name;
                hdcChild.setBackgroundColor(Color.parseColor("#66ffff"));
            }
